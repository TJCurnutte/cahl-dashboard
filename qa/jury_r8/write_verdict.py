#!/usr/bin/env python3
"""Jury R8 J5: compute weighted total (tool-verified math) and write verdict."""
import pathlib

scores = {
    "identity": (9.6, 0.12), "type": (9.5, 0.12), "color": (9.6, 0.10),
    "layout": (9.6, 0.12), "density": (9.4, 0.12), "interaction": (9.2, 0.10),
    "motion": (9.7, 0.08), "depth": (9.6, 0.08), "consistency": (9.3, 0.08),
    "mobile": (9.6, 0.08),
}
parts = {k: round(s * w, 4) for k, (s, w) in scores.items()}
total = sum(parts.values())
print("parts:", parts)
print("weighted total:", round(total, 3), "->", round(total, 1))

verdict = f"""# Jury R8 — Juror 5/5 — PRODUCT CREDIBILITY lens (v63, deployed bytes)
...
= **{round(total,1)} / 10** (unrounded {round(total,2)})
...
VERDICT: {round(total,1)}/10
"""
p = pathlib.Path.home() / "cahl-dashboard/qa/jury_r8/juror5_verdict.md"
p.write_text(verdict, encoding="utf-8")
print("wrote", p, len(verdict), "chars")
print("final line:", verdict.strip().split("\n")[-1])

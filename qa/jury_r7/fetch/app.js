    ...
    function armSkeletonWatchdog($el, label, retryExpr) {
      ...
      const wd20 = setTimeout(() => {
        if ($el.isConnected && ($el.querySelector('.skeleton') || ($el.firstElementChild && $el.firstElementChild.classList.contains('empty')))) {
          $el.innerHTML = `<div class="card"><div class="empty">Couldn\u2019t load ${label} \u2014 timed out.</div><button class="ghost small" style="margin-top:10px" onclick="${retryExpr}">Retry</button></div>`;
        }
      }, 20000);
      return () => { clearTimeout(wd8); clearTimeout(wd20); };
    }

    ...
    async function api(path, refresh=false) {
      ...
      try {
        const bust = (refresh || path.indexOf('/api/today') === 0)
          ...
          : '';
        const res = await fetch(path + bust, { cache: 'no-store', signal: ctrl.signal });
        const data = await res.json();
      ...
      if (scored.length) {
        html += '<div class="card today-card"><h2>On The Scoreboard</h2>'
          + '<div class="scored-strip">' + scored.slice(0, 4).map(g =>
            `<div class="scored-chip"><span class="scored-teams">${esc(g.home)} \u2013 ${esc(g.away)}</span><span class="scored-nums">${g.home_score}\u2013${g.away_score}</span></div>`).join('')
          + '</div></div>';
      }

      /* ---- Slate grouped by rink: scan by venue ---- */
      const byRink = {};
      upcoming.concat(finals.length ? [] : []).forEach(g => {
